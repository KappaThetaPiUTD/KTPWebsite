...
create table if not exists public.portal_events (
  ...
  check_in_passcode text not null
...
create table if not exists public.portal_attendance (
  ...
  status text not null default 'unexcused'
    check (status in ('present', 'absent', 'excused', 'unexcused')),
...
create table if not exists public.portal_attendance_logs (
  id bigint generated always as identity primary key,
  attendance_id bigint not null references public.portal_attendance(id) on delete cascade,
  event_id uuid not null references public.portal_events(id) on delete cascade,
  member_user_id uuid not null references auth.users(id) on delete restrict,
  previous_status text
    check (previous_status in ('present', 'absent', 'excused', 'unexcused')),
  new_status text not null
    check (new_status in ('present', 'absent', 'excused', 'unexcused')),
  reason text not null check (char_length(reason) between 5 and 500),
...
drop policy if exists "Members can read authorized attendance" on public.portal_attendance;
create policy "Members can read authorized attendance"
...
drop policy if exists "Admins can record attendance" on public.portal_attendance;
...
drop policy if exists "Admins can edit attendance" on public.portal_attendance;
create policy "Admins can edit attendance"
  on public.portal_attendance
  for update
  to authenticated
  using (public.is_portal_admin())
