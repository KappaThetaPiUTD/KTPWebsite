...
export default function LiveAttendanceMonitor({
  ...
  const [realtimeStatus, setRealtimeStatus] =
    useState("Connecting");
  const [updatingWindow, setUpdatingWindow] =
    useState(false);
  ...
  useEffect(() => {
    setCheckInOpen(
      Boolean(selectedEvent?.is_check_in_open)
    );
  }, [
    selectedEvent?.id,
    selectedEvent?.is_check_in_open,
  ]);
  ...
  const setCheckInWindow = async (
    ...
    try {
      const response = await fetch(
        `/api/portal/admin/events/${eventId}/check-in`,
        {
          method: "PATCH",
          headers: {
      ...
      if (!response.ok) {
        throw new Error(
          data.error ||
            "Unable to update check-in window."
        );
      }

      setCheckInOpen(
        data.event.is_check_in_open
      );
    } catch (updateError) {
      setError(
        updateError.message ||
  ...
  const openProjection = async () => {
    ...
    try {
      const response = await fetch(
        `/api/portal/admin/events/${eventId}/qr`,
        {
          cache: "no-store",
        }
      );
  ...
  return (
    <div className="mt-7 space-y-6">
      ...
      <section className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div className="w-full lg:max-w-xl">
            ...
            <select
              id="live-attendance-event"
              value={eventId}
              onChange={(event) =>
                setEventId(event.target.value)
              }
              className="mt-2 w-full rounded-lg border border-gray-300 px-4 py-3 text-gray-950 outline-none focus:border-primary focus:ring-2 focus:ring-primary/20"
            >
              <option value="">
                Choose an event
              </option>
      ...
      {showProjection && (
        <div className="fixed inset-0 z-[100] flex min-h-screen flex-col items-center justify-center bg-white p-8 text-center">
          ...
          <QRCodeSVG
            value={qrPayload}
            size={520}
            className="mt-8 max-h-[60vh] max-w-[85vw]"
            title={`QR code for ${selectedEvent?.title}`}
          />

          <p className="mt-6 text-lg text-gray-700">
            Scan this code from the Attendance page.
          </p>
          ...
          <button
            ...
            className="mt-8 rounded-lg bg-primary px-5 py-3 text-sm font-semibold text-white hover:opacity-90"
          >
            Exit projection
          </button>
        </div>
