...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    const fetchRSVPs = async () => {
      ...
      // 2) Pull the users for those RSVPs
      const userIds = Array.from(
        new Set(rsvps.map((r) => r.user_id).filter(Boolean))
      );
      const { data: users, error: usersErr } = await supabase
        .from("profiles") // <-- your people table
        .select("id, full_name, email")
        .in("id", userIds);

      if (usersErr) {
        console.error("Profiles fetch error:", usersErr);
      }

      const usersById = Object.fromEntries((users || []).map((u) => [u.id, u]));

      // (Optional) also resolve event names if event_title is missing
        ...
        <section>
          ...
          <form
            onSubmit={async (e) => {
              ...
              const visibility = e.target.visibility.value; // NEW: Get visibility
              ...
              const { error } = await supabase.from("events").insert([
                {
                  ...
                  visibility: visibility, // NEW: Add visibility
                  created_by: user?.id,
          ...
          >
            ...
            <div className="mb-4">
              ...
              <p className="text-xs text-gray-600 mt-1">
                Note: Executive members can always see all events
              </p>
            </div>
